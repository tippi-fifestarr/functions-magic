...
// this one will move us closer to the goal, explained in readme.md -- basically we add in Chainlink Price Feeds
// and OpenZeppelin NFT functionalities, this way we can charge for each character.
...
contract QuickCharacterMATICLink is VRFConsumerBaseV2Plus {
    ...
    uint256 public pricePerCharacterUSD = 20; // For testing $0.20, assuming price feed uses 18 decimal places.
    ...
    constructor() VRFConsumerBaseV2Plus(0x343300b5d84D444B2ADc9116FEF1bED02BE49Cf2)
 {
    ...
    // Function to get the cost in LINK for creating a character
    function getCharacterCostInLINK() public view returns (uint256) {
        int linkPrice = getChainlinkDataFeedLatestAnswer(); // Price of 1 LINK in USD (with 18 decimals)
        require(linkPrice > 0, "Link price is non-positive");
        return (uint256(linkPrice) / pricePerCharacterUSD) * 1e8; // Returns cost in wei
    }
